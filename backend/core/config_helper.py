...
def extract_agent_config(agent_data: Dict[str, Any], version_data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """Extract agent configuration with simplified logic for Iris vs custom agents."""
    ...
    if os.getenv("ENV_MODE", "").upper() == "STAGING":
        print(f"[DEBUG] extract_agent_config: Called for agent {agent_id}, is_iris_default={is_iris_default}")
    ...
    # Handle Iris agents with special logic
    if is_iris_default:
        return _extract_iris_agent_config(agent_data, version_data)
    
    # Handle custom agents with versioning
    return _extract_custom_agent_config(agent_data, version_data)


def _extract_iris_agent_config(agent_data: Dict[str, Any], version_data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """Extract config for Iris agents - always use central config with user customizations."""
    agent_id = agent_data.get('agent_id', 'Unknown')
    logger.debug(f"Using Iris central config for agent {agent_id}")
    
    # Start with central Iris config
    config = {
        ...
        'agentpress_tools': _extract_agentpress_tools_for_run(IRIS_CONFIG['agentpress_tools']),
        'is_default': True,
...
def build_unified_config(
    ...
    iris_metadata: Optional[Dict[str, Any]] = None,
    ...
    if iris_metadata:
        config['iris_metadata'] = iris_metadata
