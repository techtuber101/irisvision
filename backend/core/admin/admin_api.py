...
@router.post("/iris-agents/install-user/{account_id}")
async def admin_install_iris_for_user(
    account_id: str,
    replace_existing: bool = False,
    _: bool = Depends(verify_admin_api_key)
):
    """Install Iris agent for a specific user."""
    logger.debug(f"Admin installing Iris agent for user: {account_id}")
    
    service = IrisDefaultAgentService()
    agent_id = await service.install_iris_agent_for_user(account_id, replace_existing)
    
    if agent_id:
        return {
            "success": True,
            "message": f"Successfully installed Iris agent for user {account_id}",
            "agent_id": agent_id
        }
    else:
        raise HTTPException(
            status_code=500, 
            detail=f"Failed to install Iris agent for user {account_id}"
        )
