...
async def update_agent(
    ...
    try:
        ...
        is_iris_agent = agent_metadata.get('is_iris_default', False)
        restrictions = agent_metadata.get('restrictions', {})
        
        if is_iris_agent:
            logger.warning(f"Update attempt on Iris default agent {agent_id} by user {user_id}")
            
            if (agent_data.name is not None and 
                agent_data.name != existing_data.get('name') and 
                restrictions.get('name_editable') == False):
                logger.error(f"User {user_id} attempted to modify restricted name of Iris agent {agent_id}")
                raise HTTPException(
                    status_code=403, 
                    detail="Iris's name cannot be modified. This restriction is managed centrally."
                )
            
            
            if (agent_data.system_prompt is not None and 
                restrictions.get('system_prompt_editable') == False):
                logger.error(f"User {user_id} attempted to modify restricted system prompt of Iris agent {agent_id}")
                raise HTTPException(
                    status_code=403, 
                    detail="Iris's system prompt cannot be modified. This is managed centrally to ensure optimal performance."
                )
            
            if (agent_data.agentpress_tools is not None and 
                restrictions.get('tools_editable') == False):
                logger.error(f"User {user_id} attempted to modify restricted tools of Iris agent {agent_id}")
                raise HTTPException(
                    status_code=403, 
                    detail="Iris's default tools cannot be modified. These tools are optimized for Iris's capabilities."
                )
            
            if ((agent_data.configured_mcps is not None or agent_data.custom_mcps is not None) and 
                restrictions.get('mcps_editable') == False):
                logger.error(f"User {user_id} attempted to modify restricted MCPs of Iris agent {agent_id}")
                raise HTTPException(
                    status_code=403, 
                    detail="Iris's integrations cannot be modified."
                )
            
            logger.debug(f"Iris agent update validation passed for agent {agent_id} by user {user_id}")
...
async def delete_agent(agent_id: str, user_id: str = Depends(verify_and_get_user_id_from_jwt)):
    ...
    try:
        ...
        if agent.get('metadata', {}).get('is_iris_default', False):
            raise HTTPException(status_code=400, detail="Cannot delete Iris default agent")
...
async def create_agent(
    ...
    try:
        ...
        try:
            version_service = await _get_version_service()
            from .iris_config import IRIS_CONFIG
