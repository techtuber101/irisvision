...
export const ThreadContent: React.FC<ThreadContentProps> = ({
    ...
    const getAgentInfo = useCallback(() => {
        ...
        // Check if this is an Iris default agent from metadata
        const isIrisDefaultAgent = agentMetadata?.is_iris_default || false;
        ...
        if (agentData && !isIrisDefaultAgent) {
        ...
        if (recentAssistantWithAgent?.agents?.name) {
            const isIrisAgent = recentAssistantWithAgent.agents.name === 'Iris' || isIrisDefaultAgent;
            // Use modern icon system for agent display  
            const avatar = !isIrisDefaultAgent ? (
                <>
                    {isIrisAgent ? (
                        <div className="h-5 w-5 flex items-center justify-center rounded text-xs">
                            <IrisLogo size={16} />
        ...
        // Fallback: if this is an Iris default agent, always show IrisLogo
        if (isIrisDefaultAgent) {
            return {
                name: agentName || 'Iris',
                avatar: (
                    <div className="h-5 w-5 flex items-center justify-center rounded text-xs">
